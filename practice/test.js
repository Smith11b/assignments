...
// Could've used a .filter() on each and every lookup, especially for a dataset this size. but if we wanted to scale,
// using filter to parse the entire dataset each time the user input an amount would become really slow, really fast. 
// Instead, I flattened the data and created a hash table using reduce(). That way each time the user inputs
// for a new album it finds the album in 0(1) or constant time. and we only parse once while loading the app.
  function createLookup(arr){
      return arr.reduce((acc, photo) => {
        if(acc[photo.albumId] === undefined){
            acc[photo.albumId] = [{ albumId : photo.albumId, id : photo.id, title : photo.title}];
        } else {
            acc[photo.albumId].push({albumId : photo.albumId, id : photo.id, title : photo.title})
        }
        return acc;
    }, {})
}

const AlbumLookup = createLookup(arr);

console.log(AlbumLookup);
